...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--db-admin-username", help="Admin username for database creation"
    )
    parser.add_argument(
        "--db-admin-password", help="Admin password for database creation"
    )
    ...
    try:
        ...
        if result.returncode != 0:
            if "already exists" in result.stderr:
                logger.warning(
                    f"Database likely already exists: {result.stderr.strip().splitlines()[0]}"
                )
            else:
...
def load_data(args: argparse.Namespace) -> bool:
    ...
    logger.info(
        f"Data loading complete. Success: {success_count}, Failed: {fail_count}"
    )
    return fail_count == 0
